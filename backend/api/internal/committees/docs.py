...
GET_MEMBERS_DOCS: RouteDocs = {
    ...
    "description": (
        "Возвращает полный список участников орг-комитета. Фильтрация не поддерживается. "
        "Участник без привязанного аккаунта представлен позывным и тегом; "
        "при привязке к аккаунту дополнительно возвращаются user_id "
        "и ссылка на аватар пользователя."
    ),
    "status_code": status.HTTP_200_OK,
    "deprecated": False,
}
...
ADD_MEMBER_DOCS: RouteDocs = {
    ...
    "description": (
        ...
        "при регистрации игрока с таким же позывным. Если игрок уже состоит "
        "в орг-комитете - вернётся ошибка 409."
    ),
    "status_code": status.HTTP_201_CREATED,
    "deprecated": False,
}
